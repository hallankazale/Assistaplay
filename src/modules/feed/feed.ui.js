(function(global){
  ...
  function slideTemplate(item,index){const p=item.product||{};return `<article class="ap-video-slide" data-id="${esc(item.id)}" data-category="${esc(item.category)}">
    ...
    <div class="ap-like-burst" aria-hidden="true">♥</div>
  ...
  function bind(container,items){let lastTap=0;container.addEventListener('click',event=>{const slide=event.target.closest('.ap-video-slide');if(!slide)return;const button=event.target.closest('[data-action]');if(!button){const now=Date.now();if(now-lastTap<320){setLiked(slide,true);showBurst(slide);AP.events?.emit('feed:like',{contentId:slide.dataset.id,source:'double-tap'});}lastTap=now;return;}const action=button.dataset.action;const item=items.find(entry=>entry.id===slide.dataset.id);if(action==='like'){setLiked(slide,slide.dataset.liked!=='1');}if(action==='save'){const saved=slide.dataset.saved!=='1';slide.dataset.saved=saved?'1':'0';button.textContent=saved?'▣':'▱';button.classList.toggle('saved',saved);}if(action==='follow'){button.textContent=button.textContent==='✓'?'+':'✓';}if(action==='comment'){AP.feedSheets?.openComments();}if(action==='product'){AP.feedSheets?.openProduct(item?.product||{});}if(action==='share'&&navigator.share){navigator.share({title:'AssistaPay',text:'Veja este vídeo no AssistaPay'}).catch(()=>{});}AP.events?.emit(`feed:${action}`,{contentId:slide.dataset.id,category:slide.dataset.category});});}
