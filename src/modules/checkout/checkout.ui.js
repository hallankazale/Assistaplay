...
function getProduct(id){return read('seller-products',[]).find(item=>String(item.id)===String(id))||null;}
function createOrder(product,form){
  ...
  const products=read('seller-products',[]).map(item=>String(item.id)===String(product.id)?{...item,stock:Math.max(0,Number(item.stock||0)-quantity)}:item);
...
function render(container){
  ...
  container.innerHTML=`<main class="ap-page"><header class="ap-page-header"><a href="app.html?view=shop">←</a><h1>Finalizar compra</h1></header><section class="ap-card"><h2>${product.name||'Produto'}</h2><p>${money(product.price)}</p><p>Estoque: ${Number(product.stock||0)}</p></section><form id="apCheckoutForm" class="ap-card"><label>Nome<input name="name" required></label><label>E-mail<input name="email" type="email" required></label><label>CEP<input name="cep" inputmode="numeric" required></label><label>Endereço<input name="address" required></label><label>Quantidade<input name="quantity" type="number" min="1" max="${Number(product.stock||1)}" value="1" required></label><input name="affiliateId" type="hidden" value="${params.get('affiliate')||''}"><button type="submit">Confirmar pedido</button><p id="apCheckoutMessage"></p></form></main>`;
