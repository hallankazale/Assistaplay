...
function identities(el){const h=el.closest('[data-author-id],[data-user-id],[data-user],.ap-video-slide,.ap-profile-hero')||el.parentElement;const img=el.matches('img')?el:el.querySelector('img');const name=h?.querySelector?.('.ap-creator-name,[data-username],h2,strong')?.textContent||'';return [h?.dataset?.authorId,h?.dataset?.userId,h?.dataset?.user,img?.src,name].map(norm).filter(Boolean);}
function find(el){const ids=identities(el);return stories.find(s=>s.aliases.some(a=>ids.includes(a)))||null;}
