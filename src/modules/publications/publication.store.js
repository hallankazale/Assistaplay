(function(global){
  ...
  function currentAuthor(){
    ...
    let profile={};try{profile=JSON.parse(localStorage.getItem('ap:profile')||'{}');}catch{}
    return {authorId:account.id||account.email||'local-user',creator:profile.username||'@usuario',authorName:profile.name||account.name||'Usuário AssistaPay',avatar:profile.avatar||'https://i.pravatar.cc/96?img=47',bio:profile.bio||''};
  }

  async function save(input){
    const author=currentAuthor();
    const item={id:input.id||`pub_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,...author,type:input.type||'video',status:input.status||'published',createdAt:input.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString(),caption:input.caption||'',title:input.title||'',hashtags:input.hashtags||[],objective:input.objective||'Entretenimento',privacy:input.privacy||'Público',allowComments:input.allowComments!==false,allowLikes:input.allowLikes!==false,allowShares:input.allowShares!==false,productId:input.productId||'',product:input.product||null,mediaType:input.media?.type||input.mediaType||'',mediaName:input.media?.name||input.mediaName||'',mediaBlob:input.media||input.mediaBlob||null,coverBlob:input.cover||input.coverBlob||null,likes:input.likes||0,comments:input.comments||0,shares:input.shares||0,saves:input.saves||0,views:input.views||0};
  ...
  async function listByAuthor(authorId){return (await list()).filter(item=>item.authorId===authorId);}
